...
class ProductService {
    ...
    //// create product
    async createProduct(vendorId, productData, images, videos) {
        ...
        let discountPrice = productData.discountPrice || 0;
        let discountPercent = productData.discountPercent || 0;
        let salePrice = price;


        if (discountPrice > price || discountPercent > 100) {
            throw {
                status: 400,
                messege: "Discount values exceed original price limitations.",
            }
        }

        // 2. Compute missing values dynamically
        if (discountPrice > 0 && discountPercent === 0) {
            discountPercent = parseFloat(((discountPrice / price) * 100).toFixed(2));
        } else if (discountPercent > 0 && discountPrice === 0) {
            discountPrice = parseFloat(((discountPercent / 100) * price).toFixed(2));
        } else if (discountPrice > 0 && discountPercent > 0) {
            const expected = parseFloat(((discountPercent / 100) * price).toFixed(2));
            if (Math.abs(discountPrice - expected) > 0) { // 1 unit variance allowed for minor rounding discrepancies
                throw {
                    status: 400,
                    messege: "Discount values exceed original price limitations.",
                }
        ...
        salePrice = parseFloat((price - discountPrice).toFixed(2));
        ...
        /// For optional video uploading
        let productVideos = [];
        if (videos && videos.length > 0) {
            const uploadedProductVideos = await CloudinaryUpload.multipleVideoUpload(videos, "upload");
            productVideos = uploadedProductVideos.map(vid => vid.secure_url);
        }
        /// generate slug
