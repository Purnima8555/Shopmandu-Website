...
const ShopDetailPage = () => {
  ...
  const { wishlist, getWishlist, addToWishlist, removeFromWishlist } =
    useWishlistStore();
  ...
  // console.log("Shop ID:", currentShop?._id);
  useEffect(() => {
    if (!currentShop?._id) return;
    //   console.log("hi");
    getShopProducts(currentShop._id, { page: currentPage, limit: 10 }).catch(
      () => {
  ...
  useEffect(() => {
    if (!isAuthenticated || isDashboardUser) return;
    getWishlist().catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated]);
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      ...
      if (
        err?.message?.toLowerCase().includes("select a color") ||
        err?.message?.toLowerCase().includes("select a size")
      ) {
        showError(
          "This product has options to choose — please select them on the product page.",
        );
      } else {
  ...
  if (currentShopLoading) {
    return (
       <Loader className="min-h-screen" text="Loading shop..."/> 
    )
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="w-full h-40 sm:h-56 lg:h-64 bg-surface overflow-hidden">
        {banner ? (
          ...
          <div className="w-full h-full bg-linear-to-r from-primary-light to-surface" />
        )}
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="relative -mt-12 sm:-mt-14 rounded-2xl border border-border bg-card p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center gap-5">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden border border-border bg-background shrink-0 shadow-sm">
              <img
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-sm text-muted-foreground">
                ...
                {shopAddress?.city && (
                  <span className="flex items-center gap-1">
                    <FiMapPin size={14} />
                    {[shopAddress.location, shopAddress.city, shopAddress.state]
                      .filter(Boolean)
                      .join(", ")}
                  </span>
                )}
