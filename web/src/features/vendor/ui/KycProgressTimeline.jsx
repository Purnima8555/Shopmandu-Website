...
import { buildTimelineSteps, TIMELINE_DOT_CLASSES } from '../util/kycConstants';
      ...
      <div className="space-y-6 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-[1.5px] before:bg-border">
        {timelineSteps.map((step, idx) => (
          <div key={idx} className="flex gap-4 relative z-10">
            <span
              className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold border-2 ${TIMELINE_DOT_CLASSES[step.status]}`}
            >
