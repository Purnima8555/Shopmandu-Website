...
const ReturnView = ({ request, onClose }) => {
  const {
  ...
  const availableStatuses = RETURN_STATUS_TRANSITIONS[request.status] || [
    request.status,
  ];
  ...
  const handleStatusChange = async (e) => {
    ...
    if (newStatus === "APPROVED") {
      res = await sendApiRequest(() => approveReturnRequest(request._id));
    }

    if (newStatus === "REJECTED") {
      res = await sendApiRequest(() => rejectReturnRequest(request._id));
    }

    if (newStatus === "REFUNDED") {
      res = await sendApiRequest(() => refundReturnRequest(request._id));
    }
  ...
  return (
    <Modal title="Return request" onClose={onClose} maxWidth="max-w-4xl">
      <div className="space-y-6">
          ...
          <div className="flex gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-3 gap-3 rounded-xl bg-bg-main p-3">
                ...
                <Field label="Unit Price">Rs. {request.unitPrice}</Field>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <Section title="Customer">
            ...
            <Field label="Email">{request.customerId?.email || "—"}</Field>
          </Section>
        ...
        <Section title="Metadata">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Request ID">
              <span className="break-all font-mono">{request._id}</span>
            </Field>
