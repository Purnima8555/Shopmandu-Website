...
export default function Sidebar({ currentTab, setCurrentTab, isCollapsed=false, setIsCollapsed }) {
        ...
        {!isCollapsed ? (
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <div className="mt-1">
                {<KycStatusTag kycStatus={kycStatus} /> }
              </div>
            </div>
          </div>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-1 scrollbar-thin ">
        {navItems.map((item) => {
          if (item.isGroup) {
            ...
            const isOpen = isShopGroup ? shopMenuOpen : isProductsGroup ? productsMenuOpen : ordersMenuOpen;
            const setIsOpen = isShopGroup ? setShopMenuOpen : isProductsGroup ? setProductsMenuOpen : setOrdersMenuOpen;
              ...
              <div key={item.id} className="space-y-1">
                {!isCollapsed && (
                  ...
                  >
                    ...
                    <span className="text-[10px] transition-all duration-300 "> {<ChevronRight size={20} className={` ${isOpen? " rotate-90 transition-all duration-300": " rotate-0 transition-all duration-300"} `} />} </span>
                  </button>
                )}
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-[10px] text-sm font-medium transition-all group  mb-1 ${
                            isSubActive ? 'bg-primary-light text-primary font-semibold'  : 'text-text-secondary hover:bg-bg-surface hover:text-text-primary'}`}
                          // title={isCollapsed ? sub.label : ''}
                        >
                          <Icon className={`w-5 h-5 flex-shrink-0 ${isSubActive ? 'text-primary' : 'text-text-secondary group-hover:text-text-primary'}`} />
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-primary' : 'text-text-secondary group-hover:text-text-primary'}`} />
