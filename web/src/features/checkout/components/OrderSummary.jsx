    ...
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3">
            ...
            <span className="text-sm text-foreground">RS.{(item.price * item.quantity).toFixed(2)}</span>
          </div>
        ))}
      </div>
      ...
      {discount > 0 && (
        <div className="flex justify-between py-3 border-b border-border text-sm">
          ...
          <span className="text-primary font-medium">-RS.{discount.toFixed(2)}</span>
        </div>
      )}

      <div className="flex justify-between py-3 border-b border-border text-sm">
        <span className="text-muted-foreground">Total</span>
        <span className="text-foreground font-semibold text-lg">RS.{total.toFixed(2)}</span>
      </div>
